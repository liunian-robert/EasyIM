/**
 * @license eventsource.js Available under MIT License (MIT)
...
(function(global) {
    ...
    function XHRTransport(xhr, onStartCallback, onProgressCallback,
            onFinishCallback, thisArg) {
        this._internal = new XHRTransportInternal(xhr, onStartCallback,
    ...
    function XHRTransportInternal(xhr, onStartCallback, onProgressCallback,
            onFinishCallback, thisArg) {
    ...
    XHRTransportInternal.prototype.onStart = function() {
        if (this.state === 1) {
            ...
            this.onStartCallback.call(this.thisArg, status, statusText,
                    contentType);
        }
    };
    ...
    XHRTransportInternal.prototype.onTimeout2 = function() {
        ...
        var data = tmp[2] === "base64" ? global.atob(tmp[3])
                : decodeURIComponent(tmp[3]);
        if (this.state === 1) {
            this.state = 2;
    ...
    XHRTransportInternal.prototype.handleEvent = function(event) {
        if (event.type === "load") {
            ...
            // improper fix to match Firefox behaviour, but it is better than
            // just ignore abort
    ...
    XHRTransportInternal.prototype.open = function(url, withCredentials) {
        ...
        if ((!("ontimeout" in this.xhr) || ("sendAsBinary" in this.xhr) || ("mozAnon" in this.xhr))
        ...
        // withCredentials should be set after "open" for Safari and Chrome (<
        // 19 ?)
        ...
        if ("setRequestHeader" in this.xhr) {
            ...
            // Request header field Last-Event-ID is not allowed by
            // Access-Control-Allow-Headers.
    ...
    var isCORSSupported = XHR != undefined
            && (new XHR()).withCredentials != undefined;
    var Transport = isCORSSupported || (XHR != undefined && XDR == undefined) ? XHR
            : XDR;
    ...
    var getDuration = function(value, def) {
        ...
        return (n < MINIMUM_DURATION ? MINIMUM_DURATION
                : (n > MAXIMUM_DURATION ? MAXIMUM_DURATION : n));
    };
    ...
    function EventSourceInternal(es, url, options) {
        ...
        this.readyState = CONNECTING;
        this.withCredentials = isCORSSupported && options != undefined
        ...
        var CurrentTransport = options != undefined
                && options.Transport != undefined ? options.Transport
                : Transport;
        var xhr = new CurrentTransport();
        this.transport = new XHRTransport(xhr, this.onStart, this.onProgress,
                this.onFinish, this);
        this.timeout = 0;
    ...
    EventSourceInternal.prototype.onStart = function(status, statusText,
            contentType) {
        if (this.currentState === CONNECTING) {
            ...
            if (status === 200 && contentTypeRegExp.test(contentType)) {
                ...
                var message = "";
                if (status !== 200) {
                    message = "EventSource's response has a status " + status
                            + " " + statusText.replace(/\s+/g, " ")
                            + " that is not 200. Aborting the connection.";
                } else {
                    message = "EventSource's response has a Content-Type specifying an unsupported type: "
                ...
                if (this.state === AFTER_CR && c === "\n".charCodeAt(0)) {
                    ...
                    if (c === "\r".charCodeAt(0) || c === "\n".charCodeAt(0)) {
                        if (this.state !== FIELD_START) {
                            ...
                            var field = chunk.slice(this.fieldStart,
                                    this.valueStart - 1);
                            var value = chunk
                                    .slice(
                                            this.valueStart
                            ...
                            if (field === "data") {
                                ...
                                this.initialRetry = getDuration(Number(value),
                                        this.initialRetry);
                                this.retry = this.initialRetry;
                            } else if (field === "heartbeatTimeout") {
                                this.heartbeatTimeout = getDuration(
                                        Number(value), this.heartbeatTimeout);
                                if (this.timeout !== 0) {
                                    clearTimeout(this.timeout);
                                    var that = this;
                                    this.timeout = setTimeout(function() {
                        ...
                        this.state = c === "\r".charCodeAt(0) ? AFTER_CR
                                : FIELD_START;
                    } else {
                        if (this.state === FIELD_START) {
    ...
    EventSourceInternal.prototype.onTimeout = function() {
        ...
        if (this.currentState !== WAITING) {
            if (!this.wasActivity) {
                throwError(new Error("No activity within "
                        + this.heartbeatTimeout
        ...
        if (s !== "data:" && s !== "blob:") {
            s = this.url
                    + ((this.url.indexOf("?", 0) === -1 ? "?" : "&")
                            + "lastEventId="
                            + encodeURIComponent(this.lastEventId) + "&r=" + (Math
                            .random() + 1).toString().slice(2));
        } else {
            s = this.url;
        }
        try {
    ...
    var isEventSourceSupported = function() {
        ...
        return global.EventSource != undefined
